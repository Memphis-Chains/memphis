...
async function handleDoctor(context: CliContext): Promise<boolean> {
  const report = await runDoctorChecksV2({
    ...
    postInstall: context.args.postInstall,
...
function handleSlo(context: CliContext): boolean {
  const subcommand = context.args.subcommand ?? 'status';
  if (subcommand !== 'status') {
    throw new Error('slo currently supports only: memphis slo status [--days <n>] [--json]');
  }

  const result = runMemphisSloStatus({ windowDays: context.args.days }, process.env);
  if (context.args.json) {
    print(result, true);
    return true;
  }

  console.log(`SLO status: ${result.allSlosPassing ? 'passing' : 'failing'}`);
  console.log(`window: ${result.windowDays}d (${result.windowStart} - ${result.windowEnd})`);
  console.log(`samples: ${result.totalSamples}; span files: ${result.spanFilesScanned}`);
  if (result.failingSlos.length > 0) {
    console.log(`failing: ${result.failingSlos.join(', ')}`);
  }
  for (const slo of result.slos) {
    const value = slo.value === null ? 'n/a' : String(slo.value);
    const reason = slo.reason ? ` (${slo.reason})` : '';
    console.log(
      `  - ${slo.name}: ${slo.status}; value=${value}; threshold=${slo.threshold} ${slo.thresholdUnit}; samples=${slo.samples}${reason}`,
    );
  }
  return true;
}
