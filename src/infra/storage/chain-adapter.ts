...
import { loadRustBridge, parseBool } from './bridge-loader.js';
...
export function getChainAdapterStatus(rawEnv: NodeJS.ProcessEnv = process.env): ChainAdapterStatus {
  ...
  const rustBridgePath = rawEnv.RUST_CHAIN_BRIDGE_PATH ?? './crates/memphis-napi';
  ...
  const bridge = loadRustBridge(rustBridgePath);
  if (!bridge) {
    return {
      backend: 'ts-legacy',
      rustEnabled,
